...
const storeData = (cake_types, cake_stores) => {
    form.type = type.value ? type.value.id : cake_types[0].id;
    form.store = store.value ? store.value.id : cake_stores[0].id;
    form.post(route("cakes.store"), {
        errorBag: "storeData",
        preserveScroll: true,
        onError: () => {
            if (form.errors.type) {
...
<template>
    <AppLayout title="Cakes">
        ...
        <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
            <FormSection @submitted="storeData(cake_types, cake_stores)">
                <template #title>Add Cake</template>

                <template #description>
                    Add a new cake data to database.
                </template>
                        ...
                        <div class="dropdown relative mt-2 w-full">
                            ...
                            >
                                <li
                                    ...
                                    v-for="cake_type in cake_types"
                                    @click="chooseType(cake_type)"
                                >
                                    {{ cake_type.name }}
                                </li>
                            </ul>
                    ...
                    <div class="col-span-6 sm:col-span-4">
                        ...
                        <div class="dropdown relative mt-2 w-full">
                            ...
                            >
                                <li
                                    class="dropdown-item text-sm py-2 px-4 font-normal block w-full whitespace-nowrap bg-transparent text-gray-700 hover:bg-gray-100"
                                    v-for="cake_store in cake_stores"
                                    @click="chooseStore(cake_store)"
                                >
                                    {{ cake_store.name }}
                                </li>
                            </ul>
